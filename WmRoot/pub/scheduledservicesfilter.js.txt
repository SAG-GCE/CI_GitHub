...
function filterServicesInternal(){
    ...
    if (row.cells[1] != null && row.cells[1].innerHTML != "Service"){
            totalRows++;
        var source= row.cells[1].innerHTML;
        source= ReplaceTags(row.cells[1].innerHTML);
        if(isEqual(trim(source),filterCri, false)){
...
function refreshAndFilter(){
    ...
    if(is_csrf_guard_enabled && needToInsertToken) {
        document.location.replace("scheduler.dsp?filterTask=" + pfilter.value + "&statusActive=" + status.checked + "&thisISOnly=" + thisISOnly.checked + "&showAll=true&"+_csrfTokenNm_+"="+_csrfTokenVal_ );
    } else {
...
function isEqual(source, target, regEx)
{
      ...
      function redirectPage(action, oid)
      {
        ...
        if(is_csrf_guard_enabled && needToInsertToken) {
            document.location.replace(url+"&"+_csrfTokenNm_+"="+_csrfTokenVal_ );
        } else {
